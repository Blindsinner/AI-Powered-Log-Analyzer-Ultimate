...
    const patterns = [
        // ✅ ADDED: High-priority for specific installation failures with App ID
        {
...
  const [libsLoaded, setLibsLoaded] = useState(!!window.JSZip);
  // ✅ ADDED: State for AI summary (used for export)
...
  const DashboardView = ({ results, allContexts, setError, analysisSummary, isSummaryLoading }) => {
...
        switch (format) {
            case 'summary':
                if (isSummaryLoading) {
                    setError("AI Summary is still being generated.");
                    return;
                }
                if (!analysisSummary) {
                    setError("No AI Summary available. Analyze multiple errors first.");
                    return;
                }
                content = analysisSummary;
                fileName = `ai_summary_${ts}.txt`;
...
                <button onClick={() => handleExport('chart')} className="w-full text-left px-4 py-2 hover:bg-slate-600">as CHART</button>
                ...
                <button onClick={() => handleExport('summary')} className="w-full text-left px-4 py-2 hover:bg-slate-600 disabled:text-slate-500 disabled:cursor-not-allowed" disabled={!analysisSummary || isSummaryLoading}>as AI Summary (TXT)</button>
              </div>
            )}
          </div>
        </div>
...
        _       <TabButton label="Dashboard" icon={LayoutDashboard} isActive={activeTab === 'dashboard'} onClick={() => { setActiveTab('dashboard'); setIsSidebarOpen(false); }} />
...
          {activeTab === 'dashboard' && <DashboardView results={results} allContexts={allContexts} setError={setError} analysisSummary={analysisSummary} isSummaryLoading={isSummaryLoading}/>}
